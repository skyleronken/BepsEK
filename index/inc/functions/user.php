...
function hits() {
    global $dbh;
    $stmt = $dbh->prepare("SELECT count( * ) FROM hits WHERE owner = ? AND exploited = 0");
...
function countries() {
	global $dbh;
	$stmt = $dbh->prepare('SELECT country, count( * ) AS magnitude FROM hits WHERE owner = ?  AND exploited = 0 GROUP BY country ORDER BY magnitude DESC LIMIT 10');
	$stmt->execute(array($_SESSION['id']));
...
function hits_field($f = '', $exp = false) {
  ...
  $expl = ( $exp === true ) ? '1' : '0';
  $stmt = $dbh->prepare("SELECT ".$field.", count( * ) AS cc FROM hits WHERE exploited = ".$expl." AND owner = ? GROUP BY ".$field." ORDER BY id DESC LIMIT 200");
	$stmt->execute(array($_SESSION['id']));
	$hits = $stmt->fetchAll();
	foreach ($hits as $hit) {
		$row = '
...
function morris_browser_donut() {
	global $dbh;
	$stmt = $dbh->prepare('SELECT browser, count( * ) AS magnitude FROM hits  WHERE owner = ? AND exploited = 0 GROUP BY browser ORDER BY magnitude DESC LIMIT 10');
	$stmt->execute(array($_SESSION['id']));
...
function file_same($hash){
	global $dbh;
	$stmt = $dbh->prepare('SELECT COUNT(*) FROM files WHERE hash = ?');
	$stmt->execute(array($hash));
	$exist = $stmt->fetchColumn();
	if ($exist > 0) {
		return true;
